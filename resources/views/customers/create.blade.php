        ...
        <form action="{{ route('customers.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="customer_group_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        ...
                        @foreach($groups as $group)
                            <option value="{{ $group->id }}" {{ old('customer_group_id') == $group->id ? 'selected' : '' }}>{{ $group->name }} ({{ $group->discount_percentage }}% off)</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="credit_limit" value="{{ old('credit_limit', 0) }}" min="0" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                </div>
            </div>
