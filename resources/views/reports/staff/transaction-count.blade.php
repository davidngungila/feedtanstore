...
@section('page-title', 'Transaction Count')

@section('content')
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-center justify-between mb-6 gap-4">
            <h2 class="text-xl font-bold text-primary-900">Transaction Count by Cashier</h2>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('reports.staff.transaction-count.download', ['start_date' => $startDate, 'end_date' => $endDate]) }}" class="px-4 py-2 border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-50 font-medium transition-colors">
                    Export PDF
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="bg-gradient-to-br from-primary-50 to-primary-100 rounded-xl p-5">
                <div class="flex items-center justify-between mb-3">
                    <div class="w-10 h-10 rounded-lg bg-primary-200 flex items-center justify-center">
                        <i class="fas fa-receipt text-primary-700"></i>
                    </div>
                </div>
                <p class="text-sm text-primary-700 mb-1">Total Transactions</p>
                <h3 class="text-2xl font-bold text-primary-900">{{ number_format($cashiers->sum('transaction_count')) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5">
                ...
                <p class="text-sm text-blue-700 mb-1">Cashiers</p>
                <h3 class="text-2xl font-bold text-blue-900">{{ $cashiers->count() }}</h3>
            </div>
            <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-5">
                ...
                <p class="text-sm text-purple-700 mb-1">Avg. per Cashier</p>
                <h3 class="text-2xl font-bold text-purple-900">{{ number_format($cashiers->count() > 0 ? $cashiers->sum('transaction_count') / $cashiers->count() : 0, 2) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5">
                ...
                <p class="text-sm text-green-700 mb-1">Top Cashier</p>
                <h3 class="text-2xl font-bold text-green-900">{{ $cashiers->first() ? $cashiers->first()->name : 'N/A' }}</h3>
            </div>
        </div>

        <!-- Cashiers Table -->
        <div class="border border-gray-100 rounded-xl p-5">
            <h4 class="font-semibold text-primary-900 mb-4">Cashier Transactions</h4>
