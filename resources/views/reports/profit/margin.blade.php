...
@section('page-title', 'Profit Margin')

@section('content')
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-center justify-between mb-6 gap-4">
            <h2 class="text-xl font-bold text-primary-900">Profit Margin</h2>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('reports.profit.margin.download', ['start_date' => request('start_date'), 'end_date' => request('end_date')]) }}" class="px-4 py-2 border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-50 font-medium transition-colors">
                    Export PDF
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="bg-gradient-to-br from-primary-50 to-primary-100 rounded-xl p-5">
                ...
                <h3 class="text-2xl font-bold text-primary-900">TZS {{ number_format($products->sum('total_sales'), 2) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5">
                ...
                <p class="text-sm text-blue-700 mb-1">Total Profit</p>
                <h3 class="text-2xl font-bold text-blue-900">TZS {{ number_format($products->sum('profit'), 2) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5">
                ...
                <p class="text-sm text-green-700 mb-1">Avg Margin %</p>
                <h3 class="text-2xl font-bold text-green-900">{{ $products->isNotEmpty() ? round($products->avg('margin_percent'), 2) : 0 }}%</h3>
            </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right font-semibold text-green-700">TZS {{ number_format($product->profit, 2) }}</td>
                        <td class="px-4 py-3 text-right font-semibold">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $product->margin_percent >= 30 ? 'bg-green-100 text-green-700' : ($product->margin_percent >= 15 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">
