...
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-center justify-between mb-6 gap-4">
            <h2 class='text-xl font-bold text-primary-900'>Sales by Product</h2>
            <form method="GET" action="{{ route('reports.sales.by-product') }}" class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('reports.sales.by-product.download', ['start_date' => request('start_date', $startDate), 'end_date' => request('end_date', $endDate)]) }}" class="px-4 py-2 border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-50 font-medium transition-colors">Export PDF</a>
            </form>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-gray-700 font-medium">SKU</th>
                ...
                <tfoot class="bg-gray-50 font-bold">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">{{ number_format($products->sum('total_qty')) }}</td>
                        <td></td>
                        <td class="px-4 py-3 text-right">TZS {{ number_format($products->sum('total_sales'), 2) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
