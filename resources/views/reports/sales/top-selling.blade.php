...
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-center justify-between mb-6 gap-4">
            <h2 class='text-xl font-bold text-primary-900'>Top {{ $limit }} Selling Products</h2>
            <form method="GET" action="{{ route('reports.sales.top-selling') }}" class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('reports.sales.top-selling.download', ['start_date' => request('start_date', $startDate), 'end_date' => request('end_date', $endDate), 'limit' => $limit]) }}" class="px-4 py-2 border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-50 font-medium transition-colors">Export PDF</a>
            </form>
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-bold text-primary-700">{{ $index + 1 }}</td>
