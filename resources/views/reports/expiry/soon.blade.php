...
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-center justify-between mb-6 gap-4">
            <h2 class="text-xl font-bold text-primary-900">Expiring Soon</h2>
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2">
                    <label for="days-filter" class="text-sm font-medium text-gray-700">Days:</label>
                    <input type="number" name="days" id="days-filter" min="1" value="{{ request('days', 30) }}" class="form-input input-field px-3 py-2 w-24">
                </div>
                ...
                <a href="{{ route('reports.expiry.soon.download', ['days' => request('days', 30)]) }}" class="px-4 py-2 border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-50 font-medium transition-colors">
                    Export PDF
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl p-5">
                ...
                <p class="text-sm text-yellow-700 mb-1">Expiring Products</p>
            ...
            <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-5">
                <div class="flex items-center justify-between mb-3">
                    <div class="w-10 h-10 rounded-lg bg-red-200 flex items-center justify-center">
                        <i class="fas fa-dollar-sign text-red-700"></i>
                    </div>
                </div>
                <p class="text-sm text-red-700 mb-1">Total Stock Value</p>
                <h3 class="text-2xl font-bold text-red-900">TZS {{ number_format($products->sum(fn($p) => $p->quantity * $p->cost_price), 2) }}</h3>
            </div>
