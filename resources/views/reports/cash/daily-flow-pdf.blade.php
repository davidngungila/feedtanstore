...
<head>
    <meta charset="utf-8">
    <title>Daily Cash Flow Report - {{ $startDate }} to {{ $endDate }}</title>
    <style>
         body {
...
<body>
     ...
     <div class="container">
         <div class="header">
             ...
             <div class="receipt-title">DAILY CASH FLOW REPORT</div>
         </div>
         ...
         <div class="stats-grid">
             <div class="stat-card">
                 ...
                 <div class="stat-value">TZS {{ number_format($totalIncome, 2) }}</div>
             </div>
             <div class="stat-card">
                 <div class="stat-label">Total Expenses</div>
                 <div class="stat-value">TZS {{ number_format($totalExpenses, 2) }}</div>
             </div>
             <div class="stat-card">
                 <div class="stat-label">Net Cash Flow</div>
                 <div class="stat-value" style="{{ $netCashFlow < 0 ? 'color: #dc2626;' : '' }}">
                     TZS {{ number_format($netCashFlow, 2) }}</div>
             </div>
             <div class="stat-card">
         ...
         <h4 style="font-size: 12px; color: #111; font-weight: bold; margin-bottom: 10px;">Daily Breakdown:</h4>
         <table class="items-table">
             ...
             <tbody>
                 ...
                 <tr>
                     <td>{{ \Carbon\Carbon::parse($day['date'])->format('l, d F Y') }}</td>
                     <td style="text-align: right;">TZS {{ number_format($day['income'], 2) }}</td>
                     <td style="text-align: right;">TZS {{ number_format($day['expenses'], 2) }}</td>
                     <td style="text-align: right; {{ $day['net'] < 0 ? 'color: #dc2626;' : '' }}">
                         TZS {{ number_format($day['net'], 2) }}</td>
                 </tr>
                 @endforeach
