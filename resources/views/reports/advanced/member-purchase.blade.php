...
@section('page-title', 'Member Purchase Report')

@section('content')
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-center justify-between mb-6 gap-4">
            <h2 class='text-xl font-bold text-primary-900'>Member Purchase Report</h2>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('reports.advanced.member-purchase.download', ['start_date' => $startDate, 'end_date' => $endDate]) }}" class="px-4 py-2 border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-50 font-medium transition-colors">
                    Export PDF
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="bg-gradient-to-br from-primary-50 to-primary-100 rounded-xl p-5">
                ...
                <p class="text-sm text-primary-700 mb-1">Total Members</p>
                <h3 class="text-2xl font-bold text-primary-900">{{ $members->count() }}</h3>
            </div>
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5">
                ...
                <h3 class="text-2xl font-bold text-blue-900">{{ $members->sum('purchase_count') }}</h3>
            </div>
            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5">
                ...
                <h3 class="text-2xl font-bold text-green-900">TZS {{ number_format($members->sum('total_spent'), 2) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-5">
                ...
                <p class="text-sm text-purple-700 mb-1">Avg. Spent/Member</p>
                <h3 class="text-2xl font-bold text-purple-900">TZS {{ number_format($members->count() > 0 ? $members->sum('total_spent') / $members->count() : 0, 2) }}</h3>
            </div>
        </div>

        <!-- Members Table -->
        <div class="border border-gray-100 rounded-xl p-5">
            <h4 class="font-semibold text-primary-900 mb-4">Member Purchases</h4>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-3 text-left text-gray-700 font-medium">Member</th>
                    ...
                    <tbody class="divide-y">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">{{ number_format($member->purchase_count) }}</td>
                            <td class="px-4 py-3 text-right text-green-700 font-semibold">TZS {{ number_format($member->total_spent, 2) }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
