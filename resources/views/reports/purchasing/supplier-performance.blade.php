...
@section('page-title', 'Supplier Performance')

@section('content')
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6 mb-6">
        <div class="flex flex-wrap items-center justify-between mb-6 gap-4">
            <h2 class="text-xl font-bold text-primary-900">Supplier Performance</h2>
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2">
                    <label for="start_date" class="text-sm font-medium text-gray-700">From:</label>
                    <input type="date" name="start_date" id="start_date" value="{{ request('start_date', today()->subDays(90)->toDateString()) }}" class="form-input input-field px-4 py-2">
                </div>
                <div class="flex items-center gap-2">
                ...
                <a href="{{ route('reports.purchasing.supplier-performance.download', ['start_date' => request('start_date'), 'end_date' => request('end_date')]) }}" class="px-4 py-2 border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-50 font-medium transition-colors">
                    Export PDF
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5">
                ...
                <h3 class="text-2xl font-bold text-blue-900">{{ $suppliers->sum(fn($s) => $s->purchaseOrders->count()) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-5">
                ...
                <p class="text-sm text-purple-700 mb-1">Total Purchases</p>
                <h3 class="text-2xl font-bold text-purple-900">TZS {{ number_format($suppliers->sum(fn($s) => $s->purchaseOrders->sum('total')), 2) }}</h3>
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-gray-700 font-medium">Number of Orders</th>
                        <th class="px-4 py-3 text-right text-gray-700 font-medium">Total Amount</th>
                    </tr>
                </thead>
                <tbody class="divide-y">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right font-semibold">{{ $supplier->purchaseOrders->count() }}</td>
                        <td class="px-4 py-3 text-right font-semibold">TZS {{ number_format($supplier->purchaseOrders->sum('total'), 2) }}</td>
                    </tr>
                    @endforeach
