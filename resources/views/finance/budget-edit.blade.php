        ...
        <form action="{{ route('finance.budgets.update', $budget) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $budget->name) }}" required class="form-input w-full">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Description</label>
                    <textarea name="description" rows="3" class="form-input w-full">{{ old('description', $budget->description) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Budget Amount (TZS) *</label>
                    <input type="number" step="0.01" name="amount" value="{{ old('amount', $budget->amount) }}" required min="0" class="form-input w-full">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Start Date *</label>
                    <input type="date" name="start_date" value="{{ old('start_date', $budget->start_date->format('Y-m-d')) }}" required class="form-input w-full">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">End Date *</label>
                    <input type="date" name="end_date" value="{{ old('end_date', $budget->end_date->format('Y-m-d')) }}" required class="form-input w-full">
                </div>
            </div>
