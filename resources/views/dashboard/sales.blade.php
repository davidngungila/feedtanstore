...
<div class="animate-[fadeIn_0.4s_ease] space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-xl font-bold" :class="darkMode?'text-white':'text-primary-900'">Sales Dashboard</h1>
    </div>

    <!-- KPI Cards -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="card rounded-2xl p-5">
            <div class="text-sm" :class="darkMode?'text-primary-400':'text-gray-500'">Today's Revenue</div>
            <div class="text-2xl font-bold" :class="darkMode?'text-white':'text-primary-900'">TZS {{ number_format($todayRevenue, 2) }}</div>
            <div class="text-xs mt-1" :class="darkMode?'text-primary-500':'text-gray-400'">{{ $todayTransactions }} transactions</div>
        </div>
        <div class="card rounded-2xl p-5">
            <div class="text-sm" :class="darkMode?'text-primary-400':'text-gray-500'">Today's Items Sold</div>
            <div class="text-2xl font-bold" :class="darkMode?'text-white':'text-primary-900'">{{ $todayItems }}</div>
        </div>
        <div class="card rounded-2xl p-5">
            <div class="text-sm" :class="darkMode?'text-primary-400':'text-gray-500'">This Month's Revenue</div>
            <div class="text-2xl font-bold" :class="darkMode?'text-white':'text-primary-900'">TZS {{ number_format($thisMonthRevenue, 2) }}</div>
            <div class="text-xs mt-1" :class="darkMode?'text-primary-500':'text-gray-400'">{{ $thisMonthTransactions }} transactions</div>
        </div>
        <div class="card rounded-2xl p-5">
            <div class="text-sm" :class="darkMode?'text-primary-400':'text-gray-500'">This Month's Returns</div>
