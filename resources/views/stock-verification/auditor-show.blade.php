...
<div class="p-4 max-w-4xl mx-auto">
  ...
  <p class="text-sm text-gray-500">Session {{ $session->session_number }} | Status: {{ $session->status }} | All {{ $session->items->count() }} active products listed – enter only physical count. System quantity / variance are hidden.</p>
  <div class="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800"><i class="fas fa-eye-slash mr-1"></i>You must independently count physical stock. Do not see expected quantities. Leave empty → 0.</div>
  ...
  <form method="POST" action="{{ route('stock-verification.auditor-submit',$session) }}" class="card p-4 mt-4 rounded-xl">
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr>
          <td class="font-medium">{{ $it->product->name }} @if($it->product->sku)<span class="text-xs text-gray-400">({{ $it->product->sku }})</span>@endif</td>
