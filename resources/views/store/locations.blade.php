...
<div class="animate-[fadeIn_0.4s_ease]">
    <div class="card rounded-2xl p-6">
        ...
        <div class="card rounded-2xl p-6 mb-6">
            <h3 class="text-lg font-semibold text-primary-900 mb-4">Location Map</h3>
            <div id="locationsMap" class="w-full h-[400px]"></div>
        </div>
...
<script>
    ...
    const map = L.map('locationsMap').setView([centerLat, centerLng], 10);

    L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
    }).addTo(map);

    // Location type to color mapping
    const typeColors = {
        'warehouse': '#f97316',
        'store': '#22c55e',
        'other': '#6366f1'
            ...
            const marker_{{ $location->id }} = L.marker([{{ $location->latitude }}, {{ $location->longitude }}])
                ...
                .bindPopup(`
                    <div class="p-2 min-w-[200px]">
                        ...
                        <p class="text-sm text-gray-700 mb-1"><strong>Type:</strong> {{ ucfirst($location->type) }}</p>
                        ...
                        <a href="{{ route('store.locations.edit', $location) }}" class="text-primary-600 hover:underline text-sm">Edit Location</a>
                    </div>
                `);
        @endif
    @endforeach
